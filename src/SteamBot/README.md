...
You can also set config values via environment variables prefixed with `STEAMBOT_`, e.g.:
```
...
In the Discord Developer Portal, make sure your bot has:
- **`applications.commands`** scope (required for slash commands)
- **`bot`** scope
- No message content intent required (bot uses slash commands only)

### Running

```bash
cd src/SteamBot/SteamBot
dotnet run
# or release build:
...
│   └── Modules/     Slash command interaction modules
...
- Steam Web API (via plain `HttpClient` + `System.Text.Json`)
